...
In the example above, the `teams` and `roles` props will be fetched in one request, while the permissions prop will be fetched in a separate request in parallel. Group names are arbitrary strings and can be anything you choose.
...
When Inertia performs a request, it will determine which data is required and only then will it evaluate the closure. With prop modifiers, you can manually control this behavior.

Use `optional()` method to specify that a prop should never be included unless explicitly requested using the `only` client option.
