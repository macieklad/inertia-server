...
test.describe("Suite 8: Partial Reloads", () => {
	test.describe("8.1 Refresh Single Section", () => {
		test("partial reload updates only specified props", async ({ page }) => {
			...
			const initialRegularData = await page
				.getByTestId("regular-data")
				.textContent();
			...
			const newRegularData = await page
				.getByTestId("regular-data")
				.textContent();
		...
		test("always props are refreshed even when not in only list", async ({
			page,
		}) => {
			await page.goto("/always-props");
